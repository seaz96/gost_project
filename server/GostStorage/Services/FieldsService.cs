    ...
    IDocsRepository docsRepository) : IFieldsService
{
    ...
    {
        if (updatedField.Designation is not null) updatedField.Designation = TextFormattingHelper.FormatDesignation(updatedField.Designation);

        var doc = await docsRepository.GetByIdAsync(docId);

        if (doc is null) return new UnprocessableEntityObjectResult($"Document with id {docId} not found.");
